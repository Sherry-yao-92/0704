...
def process_image(filename):
    ...
    return filename, contour_image, deformations
...
def main():
    ...
    for filename, contour_image, deformations in results:
        cv2.imshow(f'Processed Image: {filename}', contour_image)
        for i, deformation in enumerate(deformations):
            print(f"Deformability for contour {i+1} in {filename}: {deformation:.6f}")
